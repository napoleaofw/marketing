...
function removeAccents(stringWithAccents) {
  var string = stringWithAccents;
	var mapAccentsHex = {
		a : /[\xE0-\xE6]/g,
		e : /[\xE8-\xEB]/g,
		i : /[\xEC-\xEF]/g,
	...
	for(var letter in mapAccentsHex) {
		var regex = mapAccentsHex[letter];
...
var formatUri = function formatUri(stringToFormat) {
	return removeAccents(stringToFormat.toLowerCase()).replace(/\s+/g, "-").replace(",", "-");
};
...
var insertList = '';
var uri = '';
$.each(categoryList, function(categoryIndex, categoryValue) {
	id++;
	idCategory = id;
	uri = formatUri(categoryIndex);
	insertList += 'insert into marketing.CATEGORY_TBL(id, category_id, name, name_uri, created_at, updated_at) values('+id+', '+'null'+', \''+categoryIndex+'\', \''+uri+'\', '+'now()'+', '+'now()'+');\n';
	$.each(categoryValue, function(subcategoryIndex, subcategoryValue) {
		id++;
		uri = formatUri(subcategoryValue);
		insertList += 'insert into marketing.CATEGORY_TBL(id, category_id, name, name_uri, created_at, updated_at) values('+id+', '+idCategory+', \''+subcategoryValue+'\', \''+uri+'\', '+'now()'+', '+'now()'+');\n';
	});
});
